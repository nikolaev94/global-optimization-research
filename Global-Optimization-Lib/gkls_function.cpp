...
double* GklsFunction::GKLS_domain_left = nullptr;

double* GklsFunction::GKLS_domain_right = nullptr;

unsigned int GklsFunction::GKLS_dim = 2;

unsigned int GklsFunction::GKLS_num_minima = 0;

double GklsFunction::GKLS_global_dist = 0.0;

double GklsFunction::GKLS_global_radius = 0.0;

double GklsFunction::GKLS_global_value = 0.0;

void GklsFunction::linear_transform(double point[])
{
	for (unsigned int i = 0; i < GKLS_dim; i++)
	{
		point[i] = point[i] * (GKLS_domain_right[i] - GKLS_domain_left[i])
			+ (GKLS_domain_right[i] + GKLS_domain_left[i]) / 2;
	}
}

GklsFunction::GklsFunction(unsigned int _sequential_number)
{
	this->sequential_number = _sequential_number;

	GKLS_minima = { nullptr };
	GKLS_glob.gm_index = nullptr;

	if (GKLS_arg_generate(sequential_number))
	{
		throw std::exception("Failed to generate GKLS function");
	}
}


GklsFunction::GklsFunction(FunctionType type, unsigned int _sequential_number)
{
	this->sequential_number = _sequential_number;
	this->func_type = type;
...
GklsFunction::GklsFunction(const GklsFunction& src)
{
	this->sequential_number = src.sequential_number;
	this->func_type = src.func_type;
...
double GklsFunction::getValue(double argument)
{
	...
	{
	case ND:
		func_value = GKLS_ND_func(point);
		break;
	case D:
		func_value = GKLS_D_func(point);
		break;
	case D2:
		func_value = GKLS_D2_func(point);
		break;
	default:
		break;
	}
...
void GklsFunction::getFunctionMinimum(double min_point[])
{
	unsigned int gm_index = GKLS_glob.gm_index[0];
	for (unsigned int i = 0; i < GKLS_dim; i++)
	{
		min_point[i] = GKLS_minima.local_min[gm_index][i];
	}
}

void GklsFunction::mapScalar_To_nDSpace(double x, double point[])
{
...
/*****************************************************************************/
double GklsFunction::GKLS_norm(double *x1, double *x2)
{
	unsigned int i;
	double norm = 0.0;
...
/*                                         local minimizers                  */
/*****************************************************************************/
int GklsFunction::GKLS_coincidence_check()
{
	unsigned int i, j;
...
/*****************************************************************************/
int GklsFunction::GKLS_set_default()
{
	unsigned int i;
	int error;
	double min_side, tmp;
...
/****************************************************************************/
int GklsFunction::GKLS_parameters_check()
{
	unsigned int i;
	double min_side, tmp;
...
/*                          parameter defining                          */
/************************************************************************/
double GklsFunction::GKLS_ND_func(double *x)
{
...
/*                          parameter defining                          */
/************************************************************************/
double GklsFunction::GKLS_D_func(double *x)
{
	unsigned int i, index;
	double norm, scal, a, rho; /* working variables */
...
/*                          parameter defining                          */
/************************************************************************/
double GklsFunction::GKLS_D2_func(double *x)
{
